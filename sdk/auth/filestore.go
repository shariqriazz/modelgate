...
func (s *FileTokenStore) readAuthFile(path, baseDir string) (*modelgateauth.Auth, error) {
	...
	if provider == "antigravity" {
		...
		tier := ""
		if t, ok := metadata["tier"].(string); ok {
			tier = strings.TrimSpace(t)
		}
		// Fetch project ID and tier if missing
		if projectID == "" || tier == "" {
			accessToken := ""
			...
			if accessToken != "" {
				discovery, errFetch := FetchAntigravityProjectAndTier(context.Background(), accessToken, http.DefaultClient)
				if errFetch == nil && discovery != nil {
					needsUpdate := false
					if projectID == "" && strings.TrimSpace(discovery.ProjectID) != "" {
						metadata["project_id"] = strings.TrimSpace(discovery.ProjectID)
						needsUpdate = true
					}
					if tier == "" && strings.TrimSpace(discovery.Tier) != "" {
						metadata["tier"] = strings.TrimSpace(discovery.Tier)
						needsUpdate = true
					}
					if needsUpdate {
